...
//method to sort ascending using full name
	public static void sortContactFullName(Contact[] savedContactArray) {
		Contact temp = new Contact();
		for (int i = 0; i < savedContactArray.length - 1; i++) {
			for (int j = i + 1; j <savedContactArray.length; j++) {
				if (savedContactArray[i].getFullName().compareToIgnoreCase(savedContactArray[j].getFullName()) > 0) {//ascending sort
...
//method to print saved contact list
